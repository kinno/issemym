 ...
 */
function ultim8magazine_form_install_configure_form_alter(&$form, $form_state) {
  $form['site_information']['site_name']['#default_value'] = 'Ultim8Magazine';
...
function _wr_profile_install_configure_form_submit($form, &$form_state) {
  ...
  drupal_set_message("Ultim8Magazine Easy Installation was a Success! Imported $count queries.");
  drupal_set_message("For security purposes, it's recommended that you delete $sql_file, or move the Ultim8Magazine profile folder out of your webroot.");
...
function _wr_profile_prefixTables($sql) {
  ...
  foreach ($prefixes as $key => $val) {
    $sql = strtr($sql, array('prefixultim8magazine_' . $key  => $val . $key));
  }
  // Then replace remaining tables with the default prefix.
  return strtr($sql, array('prefixultim8magazine_' => $defaultPrefix ));
}
